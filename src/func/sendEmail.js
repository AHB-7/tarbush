...
const sendEmail = async ({ from, phone, subject, text, fromDate, fromTime, toDate, toTime }) => {
  ...
  const dateTimeHtml = fromDate && fromTime && toDate && toTime 
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
        ...
        <h2 style="font-size: 18px; margin-bottom: 10px; color: #333;">Phone: ${phone}</h2>
  ...
  const mailOptions = {
    ...
    text: `From: ${from}\nPhone: ${phone}\n\n${text}${fromDate && fromTime && toDate && toTime ? `\n\nFra: ${fromDate} kl. ${fromTime}\nTil: ${toDate} kl. ${toTime}` : ""}`, // plain text body
    html, // html body
